...
function getRegalos() {
  try {
    return JSON.parse(localStorage.getItem(REGALOS_KEY)) || null;
  } catch (e) {
    return null;
  }
}
function setRegalos(r) {
  localStorage.setItem(REGALOS_KEY, JSON.stringify(r));
}

/* ===== WEDDING CONFIG ===== */
function getWeddingConfig() {
  try {
    return JSON.parse(localStorage.getItem(WEDDING_KEY)) || {};
  } catch (e) {
    return {};
  }
}
function setWeddingConfig(cfg) {
  localStorage.setItem(WEDDING_KEY, JSON.stringify(cfg));
}
function getWeddingDate() {
  ...
  return new Date("2027-01-31T12:00:00");
}
...
function setMusic(m) {
  localStorage.setItem(MUSIC_KEY, JSON.stringify(m));
}
function getVenue() {
  try {
    return JSON.parse(localStorage.getItem(VENUE_KEY)) || {};
  } catch (e) {
    return {};
  }
}
function setVenue(v) {
  localStorage.setItem(VENUE_KEY, JSON.stringify(v));
}
...
function updateAdminNames() {
  const cfg = getWeddingConfig();
  const bride = cfg.bride || "Cristina";
  const groom = cfg.groom || "Fernando";
...
function updateGuest(g) {
  setGuests(getGuests().map((x) => (x.id === g.id ? g : x)));
}
function deleteGuest(id) {
  setGuests(getGuests().filter((x) => x.id !== id));
}
...
function setCrono(a) {
  localStorage.setItem(CRONO_KEY, JSON.stringify(a));
}

/* ===== SEED DATA CHECK ===== */
function checkAndSeed() {
  const g = getGuests();
  if (!g.length || !Object.prototype.hasOwnProperty.call(g[0], "apellido"))
    seedDemoData();
}
...
function calcStats(guests) {
  ...
  const totalPersons = confirmed.reduce(
    (s, g) => s + 1 + (parseInt(g.acompanantes) || 0),
    0,
  );
  const pct = guests.length
    ? Math.round(((confirmed.length + rejected.length) / guests.length) * 100)
    : 0;
  return {
    total: guests.length,
  ...
  setGuests([
    {
      ...
      menu: "carne",
      restricciones: "",
      mensaje: "¡Muy emocionados!",
      notas: "Mesa principal",
      fecha: daysAgo(5),
    },
    {
      id: uid(),
    ...
    },
    {
      ...
      nombre: "Cristina",
      apellido: "Martínez",
      correo: "sofia.m@mail.com",
      telefono: "555-1005",
      acompanantes: 0,
      estado: "confirmado",
      grupo: "amigos",
      familia: "Amigos del colegio",
      mesa: 3,
      menu: "vegetariano",
      restricciones: "Vegetariana",
      mensaje: "¡No me lo perdería!",
      notas: "",
      fecha: daysAgo(7),
    },
    {
    ...
    },
    {
      ...
      nombre: "Roberto",
      apellido: "Hernández",
      correo: "roberto.h@mail.com",
      telefono: "555-1008",
      acompanantes: 0,
      estado: "rechazado",
      grupo: "trabajo",
      familia: "Trabajo Sofía",
      mesa: 0,
      menu: "",
      restricciones: "",
      mensaje: "Lo siento, no podré ir",
      notas: "Viaje",
      fecha: daysAgo(1),
    },
    {
    ...
    },
    {
      ...
      nombre: "Jorge",
      apellido: "Jiménez",
      correo: "jorge.j@mail.com",
      telefono: "555-1012",
      acompanantes: 0,
      estado: "confirmado",
      grupo: "amigos",
      familia: "Amigos universidad",
      mesa: 4,
      menu: "pescado",
      restricciones: "",
      mensaje: "",
    ...
    },
    {
      ...
      nombre: "Camila",
      apellido: "Vega",
      correo: "camila.v@mail.com",
      telefono: "555-1017",
      acompanantes: 2,
      estado: "confirmado",
      grupo: "amigos",
      familia: "Amigos del barrio",
      mesa: 5,
      menu: "vegetariano",
      restricciones: "Vegana",
      mensaje: "Con amor",
      notas: "Menú esp.",
      fecha: daysAgo(14),
    },
    {
      ...
      nombre: "Ricardo",
      apellido: "Guzmán",
      correo: "ricardo.g@mail.com",
      telefono: "555-1018",
      acompanantes: 1,
      estado: "confirmado",
      grupo: "trabajo",
      familia: "Trabajo Alejandro",
      mesa: 5,
      menu: "carne",
      restricciones: "",
      mensaje: "",
  ...
  return [
    {
      ...
      color: "#C9A96E",
      notas: "Bienvenida con champán y jazz en vivo",
    },
    {
      id: uid(),
      hora: "12:00",
      ...
      lugar: "Casa Hacienda Mamacona",
      responsable: "Juez de Registro",
      color: "#E91E63",
      notas: "Duración aprox. 40 minutos",
    },
    {
      id: uid(),
      hora: "13:00",
      ...
      lugar: "Jardín de las rosas",
      responsable: "Fotógrafo",
      color: "#9C27B0",
      notas: "Fotos familiares y con invitados",
    },
    {
      id: uid(),
      hora: "14:00",
      icon: "🍽️",
      nombre: "Cocktail & aperitivos",
      lugar: "Terraza principal",
      responsable: "Chef catering",
      color: "#FF9800",
      notas: "Bebidas y canapés de bienvenida",
    },
    {
      id: uid(),
      hora: "15:30",
      icon: "🥂",
      nombre: "Recepción & banquete",
      lugar: "Salón Imperial",
      responsable: "Maître",
      color: "#4CAF50",
      notas: "5 tiempos. Música suave de fondo",
    },
    {
      id: uid(),
      hora: "17:00",
      icon: "🎂",
      nombre: "Corte del pastel",
      lugar: "Salón Imperial",
      responsable: "Organizadora",
      color: "#F44336",
      notas: "Brindis con champán",
    },
    {
      id: uid(),
      hora: "17:30",
      icon: "💃",
      nombre: "Primer baile de los novios",
      lugar: "Pista central",
      responsable: "DJ / Banda",
      color: "#3F51B5",
      notas: '"Perfect" - Ed Sheeran',
    },
    {
      id: uid(),
      hora: "18:00",
      icon: "🎵",
      nombre: "Fiesta & baile libre",
      lugar: "Pista central",
      responsable: "DJ",
      color: "#009688",
      notas: "Playlist especial de los novios",
    },
    {
      id: uid(),
      hora: "21:00",
      icon: "🎆",
      nombre: "Show de fuegos artificiales",
      lugar: "Jardín posterior",
      responsable: "Pirotecnia",
      color: "#FF5722",
      notas: "8 minutos de espectáculo",
    },
    {
      id: uid(),
      hora: "23:00",
      icon: "🌙",
      nombre: "Cierre & despedida de novios",
      lugar: "Entrada principal",
      responsable: "Coordinadora",
      color: "#607D8B",
      notas: "Pétalos y mariposas en la salida",
    },
  ];
}
...
function chartText() {
  return darkMode ? "#c8bfa8" : "#5a4f3f";
}
function chartGrid() {
  return darkMode ? "rgba(201,169,110,.09)" : "rgba(0,0,0,.06)";
}
function chartBg() {
  return darkMode ? "#1a2035" : "#ffffff";
}
        ...
        datasets: [
          {
            ...
            data,
            borderColor: CC.gold,
            backgroundColor: "rgba(201,169,110,.12)",
            fill: true,
            tension: 0.35,
            pointRadius: 4,
...
function switchView(view) {
  ...
  document
    .querySelectorAll(".sidebar__link[data-view]")
...
function renderTable() {
  ...
  if (guestFilter !== "all")
    guests = guests.filter((g) => g.estado === guestFilter);
  if (guestGroupFilter !== "all")
    guests = guests.filter((g) => g.grupo === guestGroupFilter);
  if (search)
    guests = guests.filter((g) =>
      JSON.stringify(g).toLowerCase().includes(search),
    );
  guests.sort((a, b) => {
    const va = a[guestSort] ?? "",
      vb = b[guestSort] ?? "";
    if (!isNaN(va) && !isNaN(vb))
      return (Number(va) - Number(vb)) * guestSortDir;
  ...
  tbody.innerHTML = guests
    ...
    .join("");
  if (footer)
    footer.textContent = `${guests.length} invitado(s) · ${totalP} personas en total`;
}
...
function refreshCrono() {
  ...
  if (!timeline) return;
  timeline.innerHTML = crono.length
    ? crono
        .map(
          (ev) => `
...
function openEventoModal(id) {
  ...
  document
    ...
    .forEach((d) =>
      d.classList.toggle(
        "active",
        d.dataset.color === (ev?.color || "#C9A96E"),
      ),
    );
  modal.classList.remove("hidden");
}
...
function printCrono() {
  ...
  if (!w) return;
  w.document
    .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cronograma 2027</title>
...
function refreshStats() {
  ...
  makeDonut(
    ...
    [s.confirmed, s.pending, s.rejected],
    [CC.green, CC.amber, CC.red],
  );
  const gK = ["familia", "amigos", "trabajo", "pareja", "otro"];
  makeBar(
    "barGroupChart",
    ["Familia", "Amigos", "Trabajo", "Pareja", "Otro"],
    gK.map((g) => guests.filter((x) => x.grupo === g).length),
    [CC.pink, CC.blue, CC.green, CC.purple, CC.amber],
  );
  const mK = ["carne", "pollo", "pescado", "vegetariano", ""];
  makeBar(
    "barMenuChart",
    ["Res", "Pollo", "Pescado", "Vegetariano", "Sin elección"],
    mK.map((k) => guests.filter((g) => (g.menu || "") === k).length),
    [CC.red, CC.amber, CC.blue, CC.green, "#aaa"],
  );
  const acMap = {};
  guests.forEach((g) => {
    const n = parseInt(g.acompanantes) || 0;
    acMap[n] = (acMap[n] || 0) + 1;
  });
  ...
  guests
    ...
    .forEach((g) => {
      const d = g.fecha ? g.fecha.slice(0, 10) : "?";
...
function buildXLSXBlob(cfg) {
  ...
  function _cat() {
    const A = Array.from(arguments),
      len = A.reduce((s, a) => s + a.length, 0),
      r = new Uint8Array(len);
    let o = 0;
    A.forEach((a) => {
      r.set(a, o);
      o += a.length;
  ...
  function _crc(b) {
    let c = ~0;
    for (let i = 0; i < b.length; i++) c = (c >>> 8) ^ _CRT[(c ^ b[i]) & 255];
    return ~c >>> 0;
  }

  // ZIP STORE (sin compresión)
  function _makeZip(files) {
    const ents = files.map(([n, d]) => {
      const nm = _u8(n),
        data = typeof d === "string" ? _u8(d) : d,
        crcV = _crc(data),
        sz = data.length;
      return { nm, data, crcV, sz };
    });
    let off = 0;
    const lh = [],
      cd = [];
    ...
    const cdOff = off,
      cdSz = cd.reduce((s, p) => s + p.length, 0);
    return _cat(
      ...lh,
      ...cd,
      _cat(
        new Uint8Array([0x50, 0x4b, 5, 6]),
        _u16(0),
        _u16(0),
        _u16(ents.length),
        _u16(ents.length),
  ...
  function mTxt(v) {
    ...
    if (l.includes("res")) return "🥩 " + s;
    if (l.includes("pollo")) return "🍗 " + s;
    if (l.includes("pescado")) return "🐟 " + s;
    if (l.includes("vegetar")) return "🥗 " + s;
    return s;
  }
  ...
  const xRws = [],
    mrgs = [];
  let xRi = 1;
  function pRow(cells, ht) {
    xRws.push({ r: xRi++, cells, ht });
  }
  function mRow(val, sid, ht) {
    const r = xRi;
    pRow(
      [
        { c: 0, v: ss(String(val ?? "")), s: sid, t: "s" },
  ...
  mRow("", 4, 8);
  pRow(
    headers.map((h, i) => ({ c: i, v: ss(h), s: 5, t: "s" })),
    22,
  );
              ...
              : lv === "rechazado" || lv === "no asiste"
                ...
                : lv === "pendiente"
                  ...
                  : ev
                    ? 7
                    : 8;
          return { c: ci, v: ss(String(v || "—")), s, t: "s" };
        }
        if (ci === menuCol)
          return { c: ci, v: ss(mTxt(v)), s: ev ? 7 : 8, t: "s" };
        if (ci === 0 && groupCol === undefined)
          return {
            c: ci,
            v: typeof v === "number" ? v : ss(String(v ?? "")),
            s: ev ? 9 : 10,
            t: typeof v === "number" ? "n" : "s",
          };
        if (ci === groupCol)
          return { c: ci, v: ss(String(v ?? "")), s: ev ? 11 : 12, t: "s" };
        return {
          c: ci,
          v: typeof v === "number" ? v : ss(String(v ?? "")),
          s: ev ? 7 : 8,
      ...
      const w =
        i === (statusCol ?? -99)
          ...
          : i === (menuCol ?? -99)
            ...
            : i === (groupCol ?? -99)
              ? 18
              : Math.max(8, Math.min(h.length * 1.4, 28));
      ...
      const cx = row.cells
        .map((c) => {
          if (c.v === null || c.v === undefined)
            return `<c r="${cl(c.c)}${row.r}" s="${c.s}"/>`;
          if (c.t === "n")
            return `<c r="${cl(c.c)}${row.r}" s="${c.s}"><v>${c.v}</v></c>`;
          return `<c r="${cl(c.c)}${row.r}" s="${c.s}" t="s"><v>${c.v}</v></c>`;
        })
        .join("");
      return `<row r="${row.r}" ht="${row.ht}" customHeight="1">${cx}</row>`;
    })
    .join("");
  const MX = mrgs.length
    ? `<mergeCells count="${mrgs.length}">${mrgs.map((m) => `<mergeCell ref="${m}"/>`).join("")}</mergeCells>`
    : "";
...
function openStyledExport(cfg) {
  ...
  const brideRaw = wCfg.bride || "Cristina";
  const groomRaw = wCfg.groom || "Fernando";
  ...
  const xlsUrl = URL.createObjectURL(
    new Blob([xlsxData], {
  ...
  function cellHtml(cell, ci) {
    ...
    if (ci === statusCol) {
      ...
      if (lv === "confirmado")
        return '<span class="bdg bdg--g">✓ Confirmado</span>';
      if (lv === "rechazado" || lv === "no asiste")
        return '<span class="bdg bdg--r">✗ No asiste</span>';
      if (lv === "pendiente")
        return '<span class="bdg bdg--a">⏳ Pendiente</span>';
  ...
  const tbody = rows
    .map((row, ri) => {
      ...
      if (groupCol !== undefined) {
        ...
        if (gv !== lastGrpHtml) {
          lastGrpHtml = gv;
          grpHdr = `<tr class="grp-hdr"><td colspan="${colCount}">${esc(gv)}</td></tr>`;
        }
      }
      const cells = row
        .map((cell, ci) => {
  ...
  const w = window.open("", "_blank");
  if (!w) {
    toast("⚠️ Activa los popups del navegador para ver el reporte");
    return;
  }
...
function exportCSV() {
  ...
  openStyledExport({
    title: "Lista Completa de Invitados",
    subtitle: `${g.length} invitados registrados`,
    headers,
    rows,
    filename: `invitados_${todayStr()}.csv`,
    statusCol: 7,
    menuCol: 10,
  });
}
...
function exportAcomp() {
  ...
  openStyledExport({
    title: "Lista con Acompañantes",
    subtitle: `${g.length} confirmados · ${total} personas en total`,
    headers,
    rows,
    filename: `acompanantes_${todayStr()}.csv`,
    totalsRow: ["TOTAL CONFIRMADOS", "", "", "", total],
  });
}
...
function exportFamilia() {
  ...
  const fams = [
    ...new Set(
      g.map((x) => x.familia || grupoLabel(x.grupo) || "Sin clasificar"),
    ),
  ].sort();
  const headers = [
    "Grupo/Familia",
  ...
  fams.forEach((f) =>
    g
      .filter(
        (x) => (x.familia || grupoLabel(x.grupo) || "Sin clasificar") === f,
      )
      .forEach((x) =>
        rows.push([
          f,
  ...
  openStyledExport({
    title: "Lista por Grupo / Familia",
    subtitle: `${fams.length} grupos · ${g.length} invitados`,
    headers,
    rows,
    filename: `grupos_${todayStr()}.csv`,
    statusCol: 3,
    groupCol: 0,
  });
}
function exportJSON() {
  ...
  a.download = `boda_backup_${todayStr()}.json`;
  a.click();
...
function exportPDF() {
  ...
  const s = calcStats(g);
  const cfg = getWeddingConfig();
  const bride = esc(cfg.bride || "Cristina");
  const groom = esc(cfg.groom || "Fernando");
  const wDate = cfg.date
    ? new Date(cfg.date + "T12:00:00").toLocaleDateString("es-ES", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      })
  ...
  const w = window.open("", "_blank");
  if (!w) {
    toast("⚠️ Activa los popups del navegador para generar el reporte");
    return;
  }
...
function clearAll() {
  ...
  localStorage.removeItem(CRONO_KEY);
...
function startSidebarCountdown() {
  ...
  function tick() {
    ...
    if (diff <= 0) {
      cd.innerHTML =
        '<span style="color:var(--gold);font-size:.72rem;text-align:center;padding:4px 8px">💍 ¡Hoy es el gran día!</span>';
      return;
    }
...
function refreshConfig() {
  ...
  const ladiesNote =
    cfg.dressLadiesNote !== undefined
      ? cfg.dressLadiesNote
      : "Evitar el blanco — reservado para la novia";
  const ladiesColors = cfg.dressLadiesColors || [
    { hex: "#D4A5A5", name: "Rosa polvo" },
    { hex: "#C8A2C8", name: "Lila" },
    { hex: "#E8D5B7", name: "Champagne" },
    { hex: "#C9A96E", name: "Dorado" },
    { hex: "#B5C8C0", name: "Salvia" },
  ];
  const gentsType = cfg.dressGentsType || "Traje formal";
  const gentsNote =
    cfg.dressGentsNote !== undefined
      ? cfg.dressGentsNote
      : "Corbata o moño obligatorio";
  const gentsColors = cfg.dressGentsColors || [
    { hex: "#2C2C2C", name: "Negro" },
    { hex: "#4A4A6A", name: "Marino" },
    { hex: "#7B2D3B", name: "Burdeos" },
    { hex: "#C9A96E", name: "Dorado" },
    { hex: "#6B7C4E", name: "Verde olivo" },
  ];
  ...
  const GENTS_PRESETS = [
    ...
    { hex: "#C9A96E", name: "Dorado" },
    { hex: "#6B7C4E", name: "Verde olivo" },
    { hex: "#708090", name: "Gris pizarra" },
    { hex: "#C0C0C0", name: "Plateado" },
    { hex: "#4682B4", name: "Azul acero" },
    { hex: "#8B6914", name: "Café dorado" },
    { hex: "#36454F", name: "Gris carbón" },
    { hex: "#556B2F", name: "Oliva oscuro" },
    { hex: "#E8D5B7", name: "Champagne" },
  ];
  const dressTypesHTML = DRESS_TYPES.map(
    (t) =>
      `<option value="${t}"${t === dressCode ? " selected" : ""}>${t}</option>`,
  ).join("");
  const ladiesTypesHTML = LADIES_TYPES.map(
    (t) =>
      `<option value="${t}"${t === ladiesType ? " selected" : ""}>${t}</option>`,
  ).join("");
  const gentsTypesHTML = GENTS_TYPES.map(
    (t) =>
      `<option value="${t}"${t === gentsType ? " selected" : ""}>${t}</option>`,
  ).join("");
  const ladiesPaletteHTML = LADIES_PRESETS.map((c) => {
    const on = ladiesColors.some(
      (d) => d.hex.toLowerCase() === c.hex.toLowerCase(),
    );
    ...
    <div class="card mt-16">
      ...
      <div class="cfg-form">
        <div class="cfg-field">
          ...
          <input type="text" class="cfg-input" id="cfg-bride" value="${cfg.bride || "Cristina"}" placeholder="Ej: Sofía" maxlength="40" />
        </div>
        <div class="cfg-field">
          <label class="cfg-label"><i class="fa-solid fa-mars"></i> Nombre del novio</label>
          <input type="text" class="cfg-input" id="cfg-groom" value="${cfg.groom || "Fernando"}" placeholder="Ej: Alejandro" maxlength="40" />
        </div>
      </div>
  ...
  function updateCfgCD() {
    ...
    if (diff <= 0) {
      el.innerHTML = '<p class="cfg-day-of">💍 ¡El gran día ha llegado!</p>';
      return;
    }
    const days = Math.floor(diff / 86400000);
    const hours = Math.floor((diff % 86400000) / 3600000);
  ...
  document.getElementById("addRegalosItem")?.addEventListener("click", () => {
    ...
    renderRegalosEditor();
    document
      .getElementById("cfgRegalosList")
      .lastElementChild?.querySelector("input:last-of-type")
      ?.focus();
  });
  ...
  if (dropZone) {
    ...
    dropZone.addEventListener("dragover", (e) => {
      e.preventDefault();
      dropZone.classList.add("cfg-upload-zone--drag");
    });
  ...
  function handleAudioFileSelect(file) {
    if (file.size > 52428800) {
      toast("⚠️ El archivo supera 50 MB");
      return;
    }
    if (
      !file.type.startsWith("audio/") &&
      !file.name.match(/\.(mp3|m4a|aac|ogg|wav|flac)$/i)
    ) {
      toast("⚠️ Selecciona un archivo de audio válido");
      return;
    }
  ...
  document.getElementById("saveMusicBtn").addEventListener("click", () => {
    ...
    const url =
      (document.getElementById("cfg-music-url") || {}).value?.trim() || "";
    const localFile =
      (document.getElementById("cfg-music-local") || {}).value?.trim() || "";
    ...
    if (pendingAudioFile) {
      ...
      saveAudioFile(
        ...
        (err) => {
          ...
          setMusic({
            title: title || pendingAudioFile.name.replace(/\.[^.]+$/, ""),
            savedFile: pendingAudioFile.name,
          });
      ...
      toast(
        "⚠️ Selecciona un archivo, escribe un nombre de la carpeta music/ o ingresa una URL",
      );
    }
  });
  ...
  document.getElementById("saveDressCodeBtn").addEventListener("click", () => {
    ...
    const la = Array.from(
      document.querySelectorAll("#cfgLadiesPalette .dress-preset.active"),
    ).map((b) => ({ hex: b.dataset.hex, name: b.dataset.name }));
    const ga = Array.from(
      document.querySelectorAll("#cfgGentsPalette .dress-preset.active"),
    ).map((b) => ({ hex: b.dataset.hex, name: b.dataset.name }));
    const c = getWeddingConfig();
    setWeddingConfig({
      ...c,
      dressCode: type,
      dressLadiesType: ladiesType,
      dressLadiesNote: ladiesNote,
      dressLadiesColors: la,
      dressGentsType: gentsType,
      dressGentsNote: gentsNote,
      dressGentsColors: ga,
    });
...
function startLoginSlideshow() {
  ...
  function goToSlide(n) {
    ...
    document
      .querySelectorAll(".login-slide-dot")
...
function initDashboard() {
  ...
  document.getElementById("logoutBtn").addEventListener("click", () => {
    sessionStorage.removeItem("boda_admin_logged");
  ...
  document
    .getElementById("groupFilterSelect")
    .addEventListener("change", (e) => {
      guestGroupFilter = e.target.value;
      renderTable();
    });
  document.querySelectorAll("#view-guests .pill").forEach((pill) => {
    pill.addEventListener("click", () => {
      document
        .querySelectorAll("#view-guests .pill")
        .forEach((p) => p.classList.remove("active"));
      pill.classList.add("active");
      guestFilter = pill.dataset.filter;
      renderTable();
    });
  });
  document.querySelectorAll("#guestsTable th.sortable").forEach((th) => {
    th.addEventListener("click", () => {
      if (guestSort === th.dataset.col) guestSortDir *= -1;
      else {
        guestSort = th.dataset.col;
        guestSortDir = 1;
      }
      document.getElementById("sortSelect").value = guestSort;
      renderTable();
    });
  });
  document
    .getElementById("addGuestBtn")
    .addEventListener("click", () => openGuestModal(null));
  document.getElementById("guestModalForm").addEventListener("submit", (e) => {
    ...
    const g = {
      ...
      acompanantes:
        parseInt(document.getElementById("m-acompanantes").value) || 0,
      estado: document.getElementById("m-estado").value,
      menu: document.getElementById("m-menu").value,
      restricciones: document.getElementById("m-restricciones").value.trim(),
      grupo: document.getElementById("m-grupo").value,
      familia: document.getElementById("m-familia").value.trim(),
      mensaje: document.getElementById("m-mensaje").value.trim(),
      notas: document.getElementById("m-notas").value.trim(),
      fecha: id
        ? getGuests().find((x) => x.id === id)?.fecha ||
  ...
  /* Cronograma */
  document
    .getElementById("addEventoBtn")
    .addEventListener("click", () => openEventoModal(null));
  document
    .getElementById("exportCronoBtn")
    .addEventListener("click", printCrono);
  document.getElementById("resetCronoBtn").addEventListener("click", () => {
    if (!confirm("¿Restaurar cronograma?")) return;
    setCrono(defaultCronograma());
  ...
  document.querySelectorAll(".cp-dot").forEach((dot) => {
    dot.addEventListener("click", () => {
      ...
      document.getElementById("ev-color").value = dot.dataset.color;
    });
  });
  document.getElementById("eventoModalForm").addEventListener("submit", (e) => {
    ...
    const ev = {
      ...
      responsable: document.getElementById("ev-responsable").value.trim(),
      color: document.getElementById("ev-color").value,
      notas: document.getElementById("ev-notas").value.trim(),
    };
    const crono = getCrono();
    if (id) {
      setCrono(crono.map((x) => (x.id === id ? ev : x)));
  ...
  const closeEv = () =>
    document.getElementById("eventoModal").classList.add("hidden");
  document
    .getElementById("eventoModalClose")
    .addEventListener("click", closeEv);
  ...
  document.getElementById("eventoModal").addEventListener("click", (e) => {
    if (e.target.id === "eventoModal") closeEv();
  });
  ...
  document
    .getElementById("exportFamilia")
    .addEventListener("click", exportFamilia);
  document.getElementById("exportJSON").addEventListener("click", exportJSON);
  document.getElementById("clearData").addEventListener("click", clearAll);

  /* Mesas */
  document
    .getElementById("addMesaBtn")
    .addEventListener("click", () => openMesaModal(null));

  // Quick create mesas
  document
    .getElementById("quickCreateMesasBtn")
    ?.addEventListener("click", () => {
      document.getElementById("quickMesaModal").classList.remove("hidden");
      updateQMPreview();
    });
  const closeQM = () =>
    document.getElementById("quickMesaModal").classList.add("hidden");
  document.getElementById("quickMesaClose")?.addEventListener("click", closeQM);
  document
    .getElementById("quickMesaCancel")
    ?.addEventListener("click", closeQM);
  document.getElementById("quickMesaModal")?.addEventListener("click", (e) => {
    if (e.target.id === "quickMesaModal") closeQM();
  });
  ...
  function updateQMPreview() {
    ...
    if (el)
      el.textContent = `Se crearán: ${p} 1, ${p} 2... ${p} ${n} · ${s} asientos c/u`;
  }
  ...
  document.getElementById("quickMesaConfirm")?.addEventListener("click", () => {
    const n = Math.min(
      50,
      Math.max(1, parseInt(document.getElementById("qm-count").value) || 10),
    );
    const seats = Math.min(
      30,
      Math.max(1, parseInt(document.getElementById("qm-seats").value) || 8),
    );
  ...
  document.getElementById("mesasSidebar")?.addEventListener("click", (e) => {
    ...
    if (!tab) return;
    document
      .querySelectorAll(".ms-tab")
      .forEach((t) => t.classList.remove("active"));
    tab.classList.add("active");
    refreshMesasSidebar();
  });
  document
    .getElementById("mesasSideSearch")
    ?.addEventListener("input", refreshMesasSidebar);
  const closeMesa = () =>
    document.getElementById("mesaModal").classList.add("hidden");
  document
    .getElementById("mesaModalClose")
    .addEventListener("click", closeMesa);
  document
    .getElementById("mesaModalCancel")
    .addEventListener("click", closeMesa);
  document.getElementById("mesaModal").addEventListener("click", (e) => {
    if (e.target.id === "mesaModal") closeMesa();
  });
  document.getElementById("mesaModalForm").addEventListener("submit", (e) => {
    e.preventDefault();
    const id = document.getElementById("ms-id").value;
    const checked = Array.from(
      document.querySelectorAll("#mesaGuestsList input:checked"),
    ).map((c) => c.value);
    const mesa = {
      id: id || uid(),
  ...
  document
    .getElementById("uploadGaleriaBtn")
    .addEventListener("click", () => galeriaInput.click());
  galeriaInput.addEventListener("change", () => {
    if (galeriaInput.files.length)
      handleGaleriaUpload(Array.from(galeriaInput.files));
    galeriaInput.value = "";
  });
  ...
  galDropZone.addEventListener("dragover", (e) => {
    e.preventDefault();
    galDropZone.classList.add("galeria-upload-zone--drag");
  });
  ...
  galDropZone.addEventListener("drop", (e) => {
    ...
    if (e.dataTransfer.files.length)
      handleGaleriaUpload(Array.from(e.dataTransfer.files));
  });
  document
    .getElementById("lightboxClose")
    .addEventListener("click", () =>
      document.getElementById("galeriaLightbox").classList.add("hidden"),
    );
  document.getElementById("galeriaLightbox").addEventListener("click", (e) => {
    if (e.target.id === "galeriaLightbox")
      document.getElementById("galeriaLightbox").classList.add("hidden");
  });

  /* Presupuesto */
  document
    .getElementById("addPresupuestoBtn")
    .addEventListener("click", () => openPresupuestoModal(null));
  const closePresupuesto = () =>
    document.getElementById("presupuestoModal").classList.add("hidden");
  document
    .getElementById("presupuestoModalClose")
    .addEventListener("click", closePresupuesto);
  document
    .getElementById("presupuestoModalCancel")
    .addEventListener("click", closePresupuesto);
  document.getElementById("presupuestoModal").addEventListener("click", (e) => {
    if (e.target.id === "presupuestoModal") closePresupuesto();
  });
  document
    .getElementById("presupuestoModalForm")
  ...
  /* Proveedores */
  document
    .getElementById("addProveedorBtn")
    .addEventListener("click", () => openProveedorModal(null));
  const closeProveedor = () =>
    document.getElementById("proveedorModal").classList.add("hidden");
  document
    .getElementById("proveedorModalClose")
    .addEventListener("click", closeProveedor);
  document
    .getElementById("proveedorModalCancel")
    .addEventListener("click", closeProveedor);
  document.getElementById("proveedorModal").addEventListener("click", (e) => {
    if (e.target.id === "proveedorModal") closeProveedor();
  });
  document
    .getElementById("proveedorModalForm")
    .addEventListener("submit", (e) => {
      e.preventDefault();
...
function getMesas() {
  try {
    return JSON.parse(localStorage.getItem(MESAS_KEY)) || [];
  } catch {
    return [];
  }
}
function setMesas(a) {
  localStorage.setItem(MESAS_KEY, JSON.stringify(a));
}
...
function setPresupuesto(a) {
  localStorage.setItem(PRES_KEY, JSON.stringify(a));
}
...
function setProveedores(a) {
  localStorage.setItem(PROV_KEY, JSON.stringify(a));
}
function getGalMeta() {
  try {
    return JSON.parse(localStorage.getItem(GAL_META_KEY)) || [];
  } catch {
    return [];
  }
}
function setGalMeta(a) {
  localStorage.setItem(GAL_META_KEY, JSON.stringify(a));
}
function getMapaPos() {
  try {
    return JSON.parse(localStorage.getItem(MAPA_POS_KEY)) || {};
  } catch {
    return {};
  }
}
function setMapaPos(o) {
  localStorage.setItem(MAPA_POS_KEY, JSON.stringify(o));
}
...
function openGalDB(cb) {
  ...
  req.onupgradeneeded = (e) =>
    e.target.result.createObjectStore(GAL_STORE, { keyPath: "id" });
  req.onsuccess = (e) => cb(null, e.target.result);
  req.onerror = (e) => cb(e.target.error);
}
...
function seedMesas() {
  ...
  const guests = getGuests();
  const getIds = (fam) =>
    guests.filter((g) => g.familia === fam).map((g) => g.id);
  setMesas([
    {
      id: uid(),
      nombre: "Mesa 1",
    ...
    },
    {
      ...
      forma: "redonda",
      zona: "Amigos",
      guests: getIds("Amigos del colegio"),
    },
    {
      id: uid(),
      nombre: "Mesa 4",
      ...
      forma: "redonda",
      zona: "Amigos",
      guests: getIds("Amigos universidad"),
    },
    {
      id: uid(),
      nombre: "Mesa 5",
  ...
  setPresupuesto([
    {
      id: uid(),
      categoria: "Venue",
      concepto: "Alquiler Casa Hacienda Mamacona",
      estimado: 8000,
      real: 7500,
      pagado: "parcial",
      notas: "50% anticipo pagado",
    },
    {
      id: uid(),
      categoria: "Catering",
      concepto: "Banquete 5 tiempos + bebidas",
      estimado: 6000,
      real: 5800,
      pagado: "parcial",
      notas: "Menú degustado y aprobado",
    },
    {
      id: uid(),
      categoria: "Fotografía",
      concepto: "FotÓgrafo + videÓgrafo",
      estimado: 3000,
      real: 2800,
      pagado: "completo",
      notas: "Incluye Álbum digital",
    },
    {
      id: uid(),
      categoria: "Música",
      concepto: "DJ + equipo de sonido",
      estimado: 1500,
      real: 1400,
      pagado: "pendiente",
      notas: "",
    },
    {
      id: uid(),
      categoria: "Flores",
      concepto: "Arreglos florales y bouquet",
      estimado: 1200,
      real: 1100,
      pagado: "pendiente",
      notas: "Rosas blancas y peonías",
    },
    {
      id: uid(),
      ...
      real: 2200,
      pagado: "completo",
      notas: "Diseñadora Valentina Ruiz",
    },
    {
      id: uid(),
      categoria: "Vestimenta",
      concepto: "Traje del novio",
      estimado: 800,
      real: 750,
      pagado: "completo",
      notas: "",
    },
    {
      ...
      concepto: "Centros de mesa y arcos florales",
      estimado: 2000,
      real: 1800,
      pagado: "pendiente",
      notas: "",
    },
    {
      ...
      concepto: "Tarjetas e impresiÓn",
      estimado: 400,
      real: 380,
      pagado: "completo",
      notas: "80 invitaciones premium",
    },
    {
      id: uid(),
      categoria: "Transporte",
      concepto: "Limusina para novios",
      estimado: 600,
      real: 550,
      pagado: "pendiente",
      notas: "",
...
function seedProveedores() {
  ...
  setProveedores([
    {
      id: uid(),
      nombre: "Casa Hacienda Mamacona",
      tipo: "Venue",
      contacto: "Jorge Ríos",
      telefono: "555-2001",
      email: "info@mamacona.com",
      web: "@haciendamamacona",
      estado: "contratado",
      pago: "parcial",
      monto: 7500,
      fecha: "2027-02-14",
      notas: "Incluye jardines, salón y estacionamiento",
    },
    {
      id: uid(),
      nombre: "Catering Delicias SA",
      tipo: "Catering",
      contacto: "Chef Roberto",
      telefono: "555-2002",
      email: "catering@delicias.com",
      web: "www.delicias.com",
      estado: "contratado",
      pago: "parcial",
      monto: 5800,
      fecha: "2027-02-14",
      notas: "Menú aprobado en degustaciÓn Nov 2026",
    },
    {
      id: uid(),
      nombre: "Foto & Arte Studio",
      tipo: "Fotografía",
      contacto: "Ana Vargas",
      telefono: "555-2003",
      email: "ana@fotoyarte.com",
      web: "@fotoyarte",
      estado: "contratado",
      pago: "completo",
      monto: 2800,
      fecha: "2027-02-14",
      notas: "Álbum digital + 2 lienzos",
    },
    {
      id: uid(),
      nombre: "DJ Maestro Sounds",
      tipo: "Música / DJ",
      contacto: "DJ Carlo",
      telefono: "555-2004",
      email: "djcarlo@mail.com",
      web: "@djcarlo",
      estado: "contratado",
      pago: "pendiente",
      monto: 1400,
      fecha: "2027-02-14",
      notas: "Sonido + luces incluidos",
    },
    {
      id: uid(),
      nombre: "Flores & Sueños",
      tipo: "Flores",
      contacto: "María Peña",
      telefono: "555-2005",
      email: "flores@suenos.com",
      web: "@floresysuenos",
      estado: "contratado",
      pago: "pendiente",
      monto: 1100,
      fecha: "2027-02-14",
      notas: "Rosas blancas, peonías y eucalipto",
    },
    {
      id: uid(),
      nombre: "Pastelería Le Gâteau",
      tipo: "Pastelería",
      contacto: "Chef Sophie",
      telefono: "555-2006",
      email: "legateau@mail.com",
      web: "@legateauperu",
      estado: "cotizando",
      pago: "pendiente",
      monto: 800,
      fecha: "2027-02-14",
      notas: "Pastel 5 pisos, sabor vainilla y maracuyÁ",
    },
  ]);
}
...
function refreshDashboard() {
  ...
  const pctConf = guests.length
    ? Math.round((stats.confirmed / guests.length) * 100)
    : 0;
  const recent = [...guests]
  ...
  const provContr = getProveedores().filter(
        ...
        ${
          ...
          <li class="dash-activity-item">
            ...
            <span style="margin-left:auto;font-size:.72rem;color:var(--text-soft)">${fmtDate(g.fecha)}</span>
                ...
                .join("")
            : '<li class="dash-activity-item" style="color:var(--text-soft)">Sin actividad aún.</li>'
        }
      </ul>
    </div>`;
}
...
function refreshMesas() {
  ...
  const guests = getGuests();
  const shapeCls = {
    redonda: "mesa-shape--round",
    rectangular: "mesa-shape--rect",
    cuadrada: "mesa-shape--square",
  };
  ...
  grid.innerHTML = mesas
    .map((m) => {
      ...
      const ringCls =
        pct >= 100
          ? "mesa-ring__fill--full"
          : pct >= 75
            ? "mesa-ring__fill--near"
            : "";

      const gObjs = assigned
        .map((gid) => guests.find((x) => x.id === gid))
        .filter(Boolean);
      const SHOW = 4;
      const guestSection = gObjs.length
        ? `<div class="mesa-card__guests">${
            gObjs
              ...
              .join("") +
            (gObjs.length > SHOW
              ? `<div class="mesa-chip mesa-chip--more">+${gObjs.length - SHOW} más</div>`
              : "")
          }</div>`
...
function refreshMesasSidebar() {
  ...
  const mesaOf = {};
  mesas.forEach((m) =>
    (m.guests || []).forEach((gid) => {
      mesaOf[gid] = m.nombre;
    }),
  );

  const activeTab =
    document.querySelector(".ms-tab.active")?.dataset?.tab || "all";
  const q = (
    document.getElementById("mesasSideSearch")?.value || ""
  ).toLowerCase();

  let filtered = guests;
  if (activeTab === "sin")
    filtered = filtered.filter((g) => !assignedIds.has(g.id));
  if (q)
  ...
  if (!filtered.length) {
    listEl.innerHTML =
      '<p style="font-size:.72rem;color:var(--text-soft);padding:8px">Sin resultados</p>';
    return;
  }
...
function refreshMapa() {
  ...
  mesas.forEach((m, i) => {
    ...
    const fillClass =
      ratio >= 1 ? "--full" : ratio > 0 ? "--partial" : "--free";
    const shapeClass =
      m.forma === "rectangular"
        ? "mapa-table-inner--rect"
        : m.forma === "cuadrada"
          ? "mapa-table-inner--cuad"
          : "";
...
function handleGaleriaUpload(files) {
  const valid = files.filter(
    (f) => f.type.startsWith("image/") && f.size <= 10485760,
  );
  if (!valid.length) {
    toast("⚠️ Solo imÁgenes hasta 10 MB");
    return;
  }
  let saved = 0;
  valid.forEach((file) => {
    ...
    saveGalImage(id, file, (err) => {
      ...
      setGalMeta(metas);
      saved++;
      if (saved === valid.length) {
        toast("✅ " + saved + " imagen(es) guardada(s)");
        refreshGaleria();
      }
    });
  });
...
function refreshPresupuesto() {
  ...
  const pagados = list
    .filter((x) => x.pagado === "completo")
    .reduce((s, x) => s + x.real, 0);
  if (sumEl)
    sumEl.innerHTML = `
...
window.autoAssignGuests = function () {
  ...
  if (!mesas.length) {
    toast("⚠️ Crea mesas primero");
    return;
  }
  if (!guests.length) {
    toast("⚠️ No hay invitados confirmados");
    return;
  }
  ...
  guests.forEach((g) => {
    const key =
      g.familia && g.familia.trim() ? g.familia.trim() : "__solo__" + g.id;
  ...
  groups.forEach((group) => {
    let target = newMesas
      ...
      .sort(
        (a, b) =>
          b.capacidad - b.guests.length - (a.capacidad - a.guests.length),
      )[0];
    if (!target) {
      // Split group
      group.forEach((g) => {
        target = newMesas
          .filter((m) => m.guests.length < m.capacidad)
          .sort(
            (a, b) =>
              b.capacidad - b.guests.length - (a.capacidad - a.guests.length),
          )[0];
...
window.exportMesasPDF = function () {
  ...
  const unassigned = guests.filter(
    (g) => g.estado === "confirmado" && !allAssigned.has(g.id),
  );

  const mesaRows = mesas
    .map((m) => {
      const assigned = (m.guests || [])
  ...
  const totalPersonas = guests
    .filter((g) => g.estado === "confirmado")
    .reduce((s, g) => s + 1 + (parseInt(g.acompanantes) || 0), 0);
...
<body>
  ...
  if (!win) {
    toast("⚠️ Permite pop-ups para exportar");
    return;
  }
...
const MAPA_CATS = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        src: "img/decoracion_camino_de_flores.png",
        lbl: "Camino",
        w: 130,
        h: 50,
      },
      {
        src: "img/decoracion_cartel_de_bienbenida.png",
        lbl: "Cartel 1",
        w: 80,
        h: 80,
      },
      {
        src: "img/decoracion_cartel_de_bienbenida_2.png",
        lbl: "Cartel 2",
        w: 80,
        h: 80,
      },
      {
        src: "img/decoracion_cartel_de_bienbenida_3.png",
        lbl: "Cartel 3",
        w: 80,
        h: 80,
      },
      {
        src: "img/decoracion_corazon_de_flores.png",
        lbl: "CorazÓn",
        w: 80,
        h: 80,
      },
      ...
      {
        src: "img/decoracion_iluminaria_y_sonido.png",
        lbl: "Luces&Audio",
        w: 90,
        h: 80,
      },
      ...
      {
        src: "img/decoracion_lugar_de_regalos.png",
        lbl: "Regalos 1",
        w: 80,
        h: 80,
      },
      {
        src: "img/decoracion_lugar_de_regalos_2.png",
        lbl: "Regalos 2",
        w: 80,
        h: 80,
      },
      {
        src: "img/decoracion_lugar_de_regalos_3.png",
        lbl: "Regalos 3",
        w: 80,
        h: 80,
      },
      {
        src: "img/decoracion_lugar_de_regalos_4.png",
        lbl: "Regalos 4",
        w: 80,
        h: 80,
      },
      {
        src: "img/decoracion_mesa_decorativa.png",
        lbl: "Mesa Deco",
        w: 80,
        h: 80,
      },
...
function getMapaEls() {
  try {
    return JSON.parse(localStorage.getItem(MAPA_EL_KEY)) || [];
  } catch {
    return [];
  }
}
function setMapaEls(a) {
  localStorage.setItem(MAPA_EL_KEY, JSON.stringify(a));
}
...
function buildMapaPanel() {
  ...
  if (!catsEl) return;
  const q = (
    document.getElementById("mapaPanelSearch")?.value || ""
  ).toLowerCase();
  catsEl.innerHTML = "";
  ...
  promise
    .then((canvas) => {
      canvas.toBlob((blob) => {
        ...
        if (btn)
          btn.innerHTML =
            '<i class="fa-solid fa-image"></i><span class="mapa-btn-lbl"> Exportar PNG</span>';
        _selMapaId = prevSel;
        renderMapaEls();
      });
    })
  ...
  if (stage) {
    stage.addEventListener("pointerdown", (e) => {
      ...
      if (handle) {
        ...
        const cx =
          stageRect.left - (wrap?.scrollLeft || 0) + elData.x + elData.w / 2;
        const cy =
          stageRect.top - (wrap?.scrollTop || 0) + elData.y + elData.h / 2;
    ...
    stage.addEventListener("pointermove", (e) => {
      ...
      if (_mapaDrag.type === "move") {
        ...
        if (node) {
          node.style.left = els[idx].x + "px";
          node.style.top = els[idx].y + "px";
        }
      } else if (_mapaDrag.type === "rotate") {
        const angle =
          (Math.atan2(e.clientY - _mapaDrag.cy, e.clientX - _mapaDrag.cx) *
            180) /
          Math.PI;
        ...
        if (c === "br") {
          newW = Math.max(30, _mapaDrag.startW + dx);
          newH = Math.max(30, _mapaDrag.startH + dy);
        }
        if (c === "bl") {
          newW = Math.max(30, _mapaDrag.startW - dx);
          newH = Math.max(30, _mapaDrag.startH + dy);
          newX = _mapaDrag.startX + (_mapaDrag.startW - newW);
        }
        if (c === "tr") {
          newW = Math.max(30, _mapaDrag.startW + dx);
          newH = Math.max(30, _mapaDrag.startH - dy);
          newY = _mapaDrag.startY + (_mapaDrag.startH - newH);
        }
        if (c === "tl") {
          newW = Math.max(30, _mapaDrag.startW - dx);
          newH = Math.max(30, _mapaDrag.startH - dy);
...
/* ═══════════════════════════════════════════════════════════
   INTEGRACIÓN NEON — RSVPs del sitio web
   ═══════════════════════════════════════════════════════════ */
const NEON_RSVP_KEY = 'miboda2026admin';
const NEON_RSVP_ENDPOINT = '/api/rsvp/exclusivo-13';

async function fetchRsvpsNeon() {
  try {
    const r = await fetch(`${NEON_RSVP_ENDPOINT}?key=${NEON_RSVP_KEY}`);
    if (!r.ok) return [];
    return await r.json();
  } catch { return []; }
}

async function importarRsvpsNeon() {
  const rsvps = await fetchRsvpsNeon();
  if (!rsvps.length) return 0;

  const existentes = getGuests();
  const correosExistentes = new Set(existentes.map(g => (g.correo || '').toLowerCase()));
  let nuevos = 0;

  rsvps.forEach(rsvp => {
    const correo = (rsvp.correo || '').toLowerCase();
    if (correosExistentes.has(correo)) return; // ya importado

    const guest = {
      id: 'neon_' + rsvp.id,
      nombre: rsvp.nombre.split(' ')[0] || rsvp.nombre,
      apellido: rsvp.nombre.split(' ').slice(1).join(' ') || '',
      correo: rsvp.correo,
      telefono: rsvp.telefono || '',
      acompanantes: parseInt(rsvp.acompanantes) || 0,
      estado: rsvp.asiste ? 'confirmado' : 'rechazado',
      grupo: 'otro',
      familia: '',
      mesa: '',
      menu: '',
      ...
      notas: '🌐 Importado del sitio web',
      fecha: rsvp.fecha_registro || new Date().toISOString(),
    };
...
// Muestra el total de RSVPs de Neon en el badge del panel
async function actualizarBadgeRsvp() {
  const rsvps = await fetchRsvpsNeon();
  const badge = document.getElementById('neon-rsvp-badge');
  if (badge) badge.textContent = rsvps.length;
}

// Auto-importar al abrir el dashboard (se llama desde el evento de login)
async function onDashboardReady() {
  const nuevos = await importarRsvpsNeon();
  if (nuevos > 0) {
    // Recargar tabla de invitados si ya está visible
    if (typeof renderGuests === 'function') renderGuests(getGuests());
    if (typeof renderStats === 'function') renderStats(getGuests());
    // Mini toast
    const t = document.createElement('div');
    t.style.cssText = 'position:fixed;bottom:5rem;left:50%;transform:translateX(-50%);background:#7e8c7a;color:#fff;padding:.6rem 1.4rem;border-radius:20px;font-size:.8rem;z-index:9999;box-shadow:0 4px 16px rgba(0,0,0,.2)';
    t.textContent = `✅ ${nuevos} RSVP${nuevos > 1 ? 's' : ''} nuevo${nuevos > 1 ? 's' : ''} importado${nuevos > 1 ? 's' : ''} desde el sitio`;
    document.body.appendChild(t);
    setTimeout(() => t.remove(), 4000);
  }
}

// Hook: llamar onDashboardReady cuando el dashboard se muestre
document.addEventListener('DOMContentLoaded', () => {
  const loginForm = document.getElementById('loginForm');
  if (loginForm) {
    loginForm.addEventListener('submit', () => {
      setTimeout(onDashboardReady, 1200);
    });
  }
  // Si ya está en sesión (sessionStorage), importar igualmente
  if (sessionStorage.getItem('adminOk')) {
    setTimeout(onDashboardReady, 800);
  }
});
