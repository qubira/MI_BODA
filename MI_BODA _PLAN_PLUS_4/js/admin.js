...
function getWeddingDate() {
  ...
  return new Date("2026-02-14T18:00:00");
}
...
function getMesasConfig() {
  try {
    return JSON.parse(localStorage.getItem(MESAS_KEY)) || { silasPorMesa: 10 };
  } catch {
    return { silasPorMesa: 10 };
  }
}
function setMesasConfig(c) {
  localStorage.setItem(MESAS_KEY, JSON.stringify(c));
}
...
function getPlan() {
  try {
    const d = JSON.parse(localStorage.getItem(PLAN_KEY));
...
const VIEW_LABELS = {
  ...
  config: ["Configuración", "Ajustes generales"],
};
let currentView = "";
...
function switchView(view) {
  ...
  currentView = view;
  document
    .querySelectorAll(".view,.view--full")
...
function renderTable() {
  ...
  tbody.innerHTML = guests
    .map((g, i) => {
      ...
      const mesa = g.mesa
        ? `<span class="mesa-badge">${g.mesa}</span>`
        : `<span class="mesa-badge mesa-badge--none">—</span>`;
      return `<tr>
      <td>${i + 1}</td>
...
function openGuestModal(id) {
  ...
  document.getElementById("m-mesa").value = g?.mesa || 0;
...
function printCrono() {
  ...
  if (!w) return;
  w.document
    .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cronograma 2026</title>
  ...
  <h1>💍 Cronograma de la Boda</h1><p class="sub">Sofía &amp; Alejandro · 14 de Febrero de 2026 · Casa Hacienda Mamacona</p>
...
function refreshStats() {
  ...
  const mesaMap = {};
  guests
    .filter((g) => g.mesa > 0 && g.estado !== "rechazado")
    .forEach((g) => {
      mesaMap[g.mesa] =
        (mesaMap[g.mesa] || 0) + 1 + (parseInt(g.acompanantes) || 0);
    });
  const mNums = Object.keys(mesaMap).sort((a, b) => Number(a) - Number(b));
  makeBar(
    "barMesaChart",
    mNums.map((m) => `Mesa ${m}`),
    mNums.map((m) => mesaMap[m]),
    mNums.map(() => CC.purple),
  );
}
...
function exportMesas() {
  const g = getGuests().filter((x) => x.estado !== "rechazado" && x.mesa > 0);
  const mesas = [...new Set(g.map((x) => x.mesa))].sort((a, b) => a - b);
  const headers = [
    "Mesa",
    "Nombre",
    "Apellido",
  ...
  openStyledExport({
    title: "Lista por Mesas",
    subtitle: `${mesas.length} mesas · ${g.length} invitados asignados`,
    headers,
    rows,
    filename: `mesas_${todayStr()}.csv`,
    menuCol: 5,
    groupCol: 0,
  });
}
...
function exportPDF() {
  ...
  const mesaMap = {};
  g.filter((x) => x.estado !== "rechazado" && x.mesa > 0).forEach((x) => {
    if (!mesaMap[x.mesa]) mesaMap[x.mesa] = [];
    mesaMap[x.mesa].push(x);
  });
...
/* Mesas */
.mesas-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px}
.mesa-c{background:#faf9f6;border:1px solid #e8e2d6;border-radius:10px;border-left:4px solid #C9A96E;padding:14px 16px}
.mesa-c h5{font-size:.78rem;font-weight:700;color:#C9A96E;display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}
.mesa-c h5 span{background:rgba(201,169,110,.12);border-radius:99px;padding:1px 8px;font-size:.6rem;font-weight:600;color:#C9A96E}
.mesa-c p{font-size:.68rem;color:#777;line-height:1.9}
.mesa-c p em{color:#C9A96E;font-style:normal;font-weight:600}
/* Restricciones */
...
<div class="rpt-body">
  ...
  <div class="sec">
    ...
    <table>
      <thead><tr>
        ...
        <th>Grupo</th><th>Familia</th><th class="ta-c">Mesa</th><th>Menú</th>
      </tr></thead>
      <tbody>${g
        .map(
          (x, i) => `<tr>
  ...
  ${
    Object.keys(mesaMap).length
      ? `<div class="sec">
    <h2 class="sec-title"><span>🪑</span> Distribución por Mesas</h2>
    <div class="mesas-grid">
      ${Object.keys(mesaMap)
        ...
        .map((m) => {
          const mx = mesaMap[m];
          const tot = mx.reduce(
            (s, x) => s + 1 + (parseInt(x.acompanantes) || 0),
            0,
          );
          return `<div class="mesa-c">
          <h5>Mesa ${m} <span>${tot} pers.</span></h5>
          <p>${mx.map((x) => `${esc(x.nombre)} ${esc(x.apellido || "")}${parseInt(x.acompanantes) > 0 ? ` <em>+${x.acompanantes}</em>` : ""}`).join("<br>")}</p>
        </div>`;
        })
        .join("")}
    </div>
...
function refreshConfig() {
  ...
  document.getElementById("saveMusicBtn").addEventListener("click", () => {
    ...
    if (pendingAudioFile) {
      ...
      toast(
        "⚠️ Selecciona un archivo, ingresa una URL o el nombre del archivo local",
      );
    }
  });
...
function initDashboard() {
  ...
  document.getElementById("guestModalForm").addEventListener("submit", (e) => {
    ...
    const g = {
      ...
      mesa: parseInt(document.getElementById("m-mesa").value) || 0,
  ...
  document.getElementById("exportMesas").addEventListener("click", exportMesas);
  document
