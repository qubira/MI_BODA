...
function makeQR(text) {
  ...
  const cell=14,grid=10; ctx.fillStyle='#1a5276';
  ...
  [[0,0],[0,7],[7,0]].forEach(([cr,cc])=>{ctx.fillStyle='#1a5276';ctx.fillRect(cc*cell,cr*cell,3*cell-2,3*cell-2);ctx.fillStyle='#fff';ctx.fillRect(cc*cell+2,cr*cell+2,3*cell-6,3*cell-6);ctx.fillStyle='#c4a35a';ctx.fillRect(cc*cell+5,cr*cell+5,3*cell-12,3*cell-12);});
  return c;
}
function spawnConfetti(){const box=document.getElementById('rsvpConfetti');if(!box)return;box.innerHTML='';const colors=['#1a5276','#c4a35a','#fff','#f0ece3','#d0c8b8'];for(let i=0;i<28;i++){const p=document.createElement('div');p.className='confetti-piece';p.style.cssText=`left:${Math.random()*100}%;background:${colors[Math.floor(Math.random()*colors.length)]};animation-delay:${Math.random()*.7}s;animation-duration:${1.4+Math.random()*.6}s;border-radius:${Math.random()>.5?'50%':'2px'};`;box.appendChild(p);}}
