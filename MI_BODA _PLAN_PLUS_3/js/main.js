...
(function initIntro() {
  ...
  /* Primer clic: las solapas del sobre se abren y la carta emerge
     revelando su contenido en cascada, para poder apreciarla con calma */
  function openLetter() {
    if (opened) return;
    opened = true;
...
(function renderCronograma() {
  ...
  const CRONO_DEFAULTS = [
    {hora:'11:00',icon:'💐',nombre:'Llegada de invitados',         lugar:'Jardín principal',        color:'#b79b66',notas:'Bienvenida con champán y jazz en vivo'},
...
function makeQR(text) {
  ...
  [[0,0],[0,7],[7,0]].forEach(([cr, cc]) => {
    ctx.fillStyle = '#4a3a23';  ctx.fillRect(cc*cell, cr*cell, 3*cell-2, 3*cell-2);
    ctx.fillStyle = '#FFF';     ctx.fillRect(cc*cell+2, cr*cell+2, 3*cell-6, 3*cell-6);
    ctx.fillStyle = '#b79b66';  ctx.fillRect(cc*cell+5, cr*cell+5, 3*cell-12, 3*cell-12);
  });
...
function spawnConfetti() {
  ...
  if (!container) return;
  container.innerHTML = '';
  const colors = ['#b79b66','#e9dab9','#c9a07a','#e9d9bd','#8a6f43','#FFF'];
  for (let i = 0; i < 30; i++) {
