...
(function initCursor() {
  ...
  document.querySelectorAll('a, button, label, .th-item__media, .mg-item, .gift-card, .padrino-card, .faq-item summary, .rsvp-card, .menu-opt').forEach(el => {
...
/* ===== SCROLL REVEAL (varios estilos, con stagger por grupo) ===== */
(function initReveal() {
  const SELECTOR = '.reveal, .reveal-left, .reveal-right, .reveal-zoom, .reveal-rotate, .reveal-clip';
  const groups = new Map(); // parentEl -> contador para el stagger

  document.querySelectorAll(SELECTOR).forEach(el => {
    const parent = el.parentElement;
    const n = groups.get(parent) || 0;
    el.style.setProperty('--stag', n);
...
/* ===== TEXTO QUE SE REVELA PALABRA / LETRA A LETRA ===== */
...
(function initSplitReveal() {
  ...
  els.forEach(el => splitText(el, el.getAttribute('data-split')));
...
/* Letras del nombre de los novios: se revelan al abrir la invitación */
(function initHeroNameSplit() {
  const bride = document.getElementById('heroBride');
  const groom = document.getElementById('heroGroom');
  [bride, groom].forEach(el => splitText(el, 'letter'));
  window.__revealHeroNames = function () {
    if (bride) bride.classList.add('split-in');
    if (groom) setTimeout(() => groom.classList.add('split-in'), 120);
  };
})();

/* ===== PARALLAX SUAVE EN FONDOS DE SECCIÓN ===== */
function initBgParallax(selector, factor) {
...
initBgParallax('.timeline-bg', .15);
initBgParallax('.verse-section__bg', .08);
...
/* ===== RSVP — verificación por nombre + apellido ===== */
(function initRSVP() {
  const form = document.getElementById('rsvpForm');
  if (!form) return;

  const s1    = document.getElementById('step1');
  const sConf = document.getElementById('stepConfirm');
  const sok   = document.getElementById('stepSuccess');
  let found   = null;
  ...
  function showStep(el) {
    [s1,sConf,sok].forEach(s => s && s.classList.remove('active'));
  ...
  document.getElementById('rsvpVerify').addEventListener('click', function() {
    const nom = (document.getElementById('rNombre').value||'').trim();
    const ape = (document.getElementById('rApellido').value||'').trim();
    const err = document.getElementById('errVerify');
    if (!nom || nom.length < 2) { if(err) err.textContent='Ingresa tu nombre.'; return; }
    if (!ape || ape.length < 2) { if(err) err.textContent='Ingresa tu apellido.'; return; }
    if (err) err.textContent = '';
    found = findGuest(`${nom} ${ape}`);
    if (!found) {
      if (err) err.textContent = 'Nombre no encontrado. Verifica que nombre y apellido estén escritos exactamente como aparecen en tu invitación.';
      return;
    }
    ...
    const giftRow = document.getElementById('rsvpGiftRow');
  ...
  function confirmAttend(asist) {
    ...
    const rec = {
      ...
      companions: parseInt(found.acompanantes)||0,
      asistencia: asist,
    ...
    if (isYes) {
      ...
      qrEl.appendChild(makeQR(`EXCL2-${rec.id}-${rec.nombre}`));
  ...
  document.getElementById('resetForm').addEventListener('click', function() {
    found = null;
    const nEl = document.getElementById('rNombre'); if(nEl) nEl.value = '';
    const aEl = document.getElementById('rApellido'); if(aEl) aEl.value = '';
    const err = document.getElementById('errVerify'); if(err) err.textContent = '';
    const qrEl = document.getElementById('successQR'); if(qrEl){ qrEl.innerHTML=''; qrEl.style.display=''; }
    const qrNote = document.getElementById('qrNote'); if(qrNote) qrNote.style.display = '';
    showStep(s1);
  });
  ...
  if (nEl) {
    ...
    nEl.addEventListener('keydown', e => { if(e.key==='Enter'){e.preventDefault(); document.getElementById('rApellido')?.focus();} });
  }
  const aEl2 = document.getElementById('rApellido');
  if (aEl2) {
    aEl2.addEventListener('blur', function() {
      this.value = this.value.replace(/\s+/g,' ').trim()
        .replace(/\S+/g, w => w[0].toUpperCase()+w.slice(1).toLowerCase());
    });
    aEl2.addEventListener('keydown', e => { if(e.key==='Enter'){e.preventDefault(); document.getElementById('rsvpVerify').click();} });
  }
})();
...
(function initLightbox() {
  const items = document.querySelectorAll('.th-item__media, .mg-item');
