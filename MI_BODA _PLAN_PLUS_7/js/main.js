...
function makeQR(text) {
  ...
  const cell=14, grid=10; ctx.fillStyle='#611128';
  ...
  [[0,0],[0,7],[7,0]].forEach(([cr,cc])=>{
    ctx.fillStyle='#611128'; ctx.fillRect(cc*cell,cr*cell,3*cell-2,3*cell-2);
    ctx.fillStyle='#fff'; ctx.fillRect(cc*cell+2,cr*cell+2,3*cell-6,3*cell-6);
    ctx.fillStyle='#C9A96A'; ctx.fillRect(cc*cell+5,cr*cell+5,3*cell-12,3*cell-12);
  });
  return c;
}
...
function spawnConfetti() {
  ...
  const colors=['#611128','#C9A96A','#fff','#f0e8e0','#c0b0a0'];
  for(let i=0;i<28;i++){
...
(function initRSVP(){
  ...
  form.addEventListener('submit',e=>{
    ...
    document.getElementById('rsvpOkMsg').textContent=isYes?`¡${guest.nombre}, te esperamos el 14 de Febrero de 2027!`:`${guest.nombre}, lamentamos que no puedas estar — tus buenos deseos nos alegran.`;
