...
function makeQR(text) {
  ...
  [[0,0],[0,7],[7,0]].forEach(([cr, cc]) => {
    ctx.fillStyle = '#1A1817'; ctx.fillRect(cc*cell, cr*cell, 3*cell-2, 3*cell-2);
    ctx.fillStyle = '#fff';    ctx.fillRect(cc*cell+2, cr*cell+2, 3*cell-6, 3*cell-6);
    ctx.fillStyle = '#C8BEB0'; ctx.fillRect(cc*cell+5, cr*cell+5, 3*cell-12, 3*cell-12);
  });
  return c;
}
...
function spawnConfetti() {
  ...
  const colors = ['#1A1817','#C8BEB0','#fff','#e8e0d0','#a0a090'];
  for (let i = 0; i < 28; i++) {
