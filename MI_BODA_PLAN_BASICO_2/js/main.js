...
/* ===== INTRO — CARTA DE INVITACIÓN (sobre) ===== */
(function initIntro() {
  var overlay = document.getElementById('introOverlay');
  if (!overlay) return;

  var brideInitEl = document.getElementById('introCardBride');
  var groomInitEl = document.getElementById('introCardGroom');
  var brideNameEl = document.getElementById('introCardBrideName');
  var groomNameEl = document.getElementById('introCardGroomName');
  var dateEl      = document.getElementById('introCardDate');

  if (brideInitEl) brideInitEl.textContent = WEDDING.bride.charAt(0).toUpperCase();
  if (groomInitEl) groomInitEl.textContent = WEDDING.groom.charAt(0).toUpperCase();
  if (brideNameEl) brideNameEl.textContent = WEDDING.bride;
  if (groomNameEl) groomNameEl.textContent = WEDDING.groom;
  if (dateEl) {
  ...
  function open() {
    ...
    /* Secuencia: sello se rompe (.5s) → solapa abre en 3D (1.05s) → carta se eleva (.28s delay + 1.3s) → fundido del overlay (delay 1.62s + .6s) */
    setTimeout(function() { overlay.style.display = 'none'; }, 2250);
  }
...
(function updateWeddingNames() {
  ...
  // Iniciales en nav (encabezado) y monograma del hero
  ...
  const elHeroMonoBride = document.getElementById('navBrideInitial2');
  const elHeroMonoGroom = document.getElementById('navGroomInitial2');
  if (elHeroMonoBride) elHeroMonoBride.textContent = bi;
  if (elHeroMonoGroom) elHeroMonoGroom.textContent = gi;
