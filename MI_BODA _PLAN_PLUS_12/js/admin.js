...
function getMesasConfig() { try { return JSON.parse(localStorage.getItem(MESAS_KEY))||{silasPorMesa:10}; } catch { return {silasPorMesa:10}; } }
function setMesasConfig(c){ localStorage.setItem(MESAS_KEY, JSON.stringify(c)); }
...
function getPlan() { try { const d=JSON.parse(localStorage.getItem(PLAN_KEY)); return (d&&d.items)?d:{items:[]}; } catch { return {items:[]}; } }
function setPlan(p){ localStorage.setItem(PLAN_KEY, JSON.stringify(p)); }
...
function defaultCronograma() {
  return [
    ...
    {id:uid(),hora:'12:00',icon:'💍',nombre:'Ceremonia civil',             lugar:'Capilla de Los Laureles', responsable:'Juez de Registro', color:'#E91E63',notas:'Duración aprox. 40 minutos'},
...
function renderTable() {
  ...
  tbody.innerHTML = guests.map((g,i)=>{
    ...
    const mesa=g.mesa?`<span class="mesa-badge">${g.mesa}</span>`:`<span class="mesa-badge mesa-badge--none">—</span>`;
    return `<tr>
      <td>${i+1}</td>
...
function printCrono() {
  ...
  if (!w) return;
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cronograma 2026</title>
...
function refreshStats() {
  ...
  const mesaMap={};
  guests.filter(g=>g.mesa>0&&g.estado!=='rechazado').forEach(g=>{mesaMap[g.mesa]=(mesaMap[g.mesa]||0)+1+(parseInt(g.acompanantes)||0);});
  const mNums=Object.keys(mesaMap).sort((a,b)=>Number(a)-Number(b));
  makeBar('barMesaChart',mNums.map(m=>`Mesa ${m}`),mNums.map(m=>mesaMap[m]),mNums.map(()=>CC.purple));
}
...
function exportCSV() {
  const g = getGuests();
  const headers = ['#','Nombre','Apellido','Correo','Teléfono','Acomp.','Total','Estado','Grupo','Familia/Grupo','Mesa','Menú','Restricciones','Mensaje','Notas','Fecha'];
  const rows = g.map((x, i) => [i+1, x.nombre, x.apellido||'', x.correo||'', x.telefono||'', x.acompanantes||0, 1+(parseInt(x.acompanantes)||0), estadoLabel(x.estado), grupoLabel(x.grupo), x.familia||'', x.mesa||'', menuLabel(x.menu), x.restricciones||'', x.mensaje||'', x.notas||'', fmtDate(x.fecha)]);
  openStyledExport({ title:'Lista Completa de Invitados', subtitle:`${g.length} invitados registrados`, headers, rows, filename:`invitados_${todayStr()}.csv`, statusCol:7, menuCol:11 });
}
...
function exportAcomp() {
  ...
  const headers = ['Nombre','Apellido','Grupo','Acomp.','Total Personas','Mesa'];
  const rows = g.map(x => [x.nombre, x.apellido||'', grupoLabel(x.grupo), x.acompanantes||0, 1+(parseInt(x.acompanantes)||0), x.mesa||'Sin asignar']);
...
function exportMesas() {
  ...
  const headers = ['Mesa','Nombre','Apellido','Acomp.','Total','Menú','Restricciones'];
  const rows = [];
  mesas.forEach(m => g.filter(x => x.mesa === m).forEach(x => rows.push([`Mesa ${m}`, x.nombre, x.apellido||'', x.acompanantes||0, 1+(parseInt(x.acompanantes)||0), menuLabel(x.menu), x.restricciones||''])));
  openStyledExport({ title:'Lista por Mesas', subtitle:`${mesas.length} mesas · ${g.length} invitados asignados`, headers, rows, filename:`mesas_${todayStr()}.csv`, menuCol:5, groupCol:0 });
}

function exportFamilia() {
  const g = getGuests();
  const fams = [...new Set(g.map(x => x.familia || grupoLabel(x.grupo) || 'Sin clasificar'))].sort();
  const headers = ['Grupo/Familia','Nombre','Apellido','Estado','Acomp.','Total','Mesa'];
  const rows = [];
  fams.forEach(f => g.filter(x => (x.familia || grupoLabel(x.grupo) || 'Sin clasificar') === f).forEach(x => rows.push([f, x.nombre, x.apellido||'', estadoLabel(x.estado), x.acompanantes||0, 1+(parseInt(x.acompanantes)||0), x.mesa||''])));
...
function exportJSON() {
  const data={guests:getGuests(),cronograma:getCrono(),mesasConfig:getMesasConfig(),plan:getPlan(),exportDate:new Date().toISOString()};
...
<div class="rpt-body">
  ...
  ${Object.keys(mesaMap).length ? `<div class="sec">
    <h2 class="sec-title"><span>🪑</span> Distribución por Mesas</h2>
    <div class="mesas-grid">
      ${Object.keys(mesaMap).sort((a,b)=>Number(a)-Number(b)).map(m=>{
        const mx=mesaMap[m];
        const tot=mx.reduce((s,x)=>s+1+(parseInt(x.acompanantes)||0),0);
...
function clearAll() {
  ...
  localStorage.removeItem(DB_KEY); localStorage.removeItem(CRONO_KEY); localStorage.removeItem(PLAN_KEY);
